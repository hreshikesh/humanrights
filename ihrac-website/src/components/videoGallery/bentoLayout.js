...
export const generateBentoLayout = (total) => {
  ...
  if (total === 1) return [{ cols: "col-span-12", rows: "row-span-1 md:row-span-2 h-full" }];
  if (total === 2) return [
    { cols: "col-span-12 md:col-span-8", rows: "row-span-1 md:row-span-2 h-full" },
    { cols: "col-span-12 md:col-span-4", rows: "row-span-1 md:row-span-2 h-full" }
  ];
  if (total === 3) return [
    { cols: "col-span-12 md:col-span-8", rows: "row-span-1 md:row-span-2 h-full" },
  ...
  for (let i = 0; i < total; i++) {
    const cycle = i % 5;
    if (cycle === 0) layouts.push({ cols: "col-span-12 md:col-span-8", rows: "row-span-1 md:row-span-2 h-full" });
