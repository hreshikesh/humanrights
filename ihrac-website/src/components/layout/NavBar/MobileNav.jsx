  ...
  return (
    <>
      ...
      <div className="lg:hidden h-20 px-5 flex items-center justify-between bg-white shadow-sm sticky top-0 z-40 border-b border-slate-100">
        ...
        <button onClick={() => setIsOpen(true)} className="text-[#0B1F3A] p-2 hover:bg-slate-50 rounded-md transition-colors">
          <Menu size={28} />
        </button>
      </div>
          ...
          <>
            ...
            >
              ...
              <div className="flex justify-between items-center p-5 border-b border-slate-100 bg-slate-50">
                <h2 className="font-bold text-[15px] tracking-widest uppercase text-[#0B1F3A]">Navigation</h2>
                <button
                  onClick={() => setIsOpen(false)}
                    ...
                    <div key={item.title} className="border-b border-slate-50 last:border-none">
                      {!hasChildren ? (
                        <NavLink
                          ...
                          onClick={() => setIsOpen(false)}
                          className={({ isActive }) =>
                            `block py-3.5 px-4 rounded-md font-semibold text-[15px] transition-colors ${
                              isActive
                          ...
                          >
                            ...
                            <ChevronDown
                              ...
                              className={`transition-transform duration-300 ${
                                openDropdown === item.title ? "rotate-180 text-[#D4AF37]" : "text-slate-400"
                              }`}
                            />
                          </button>
                                ...
                                <div className="pl-4 pr-2 py-2 mb-2 flex flex-col gap-1 border-l-2 border-slate-200 ml-6">
                                  {childrenArray.map((sub) => (
                                    ...
                                    >
                                      ...
                                      {sub.desc && (
                                        <span className="text-[12px] text-slate-400 mt-0.5">{sub.desc}</span>
                                      )}
                                    </NavLink>
                                  ))}
                                  
                                  {/* Mobile Explore All CTA for Working Sectors */}
                                  {item.megaMenu && (
                                    <NavLink
                                      to={item.path}
                                      onClick={() => setIsOpen(false)}
